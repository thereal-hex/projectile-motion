...
class EnhancedProjectileSimulator:
    def __init__(self, root):
        ...
        self.root.geometry("1200x700")  # Increased width from 1000 to 1200
        ...
        left_panel_frame = tk.Frame(main_frame, bg="#2c3e50", width=350)  # Increased width from default
        left_panel_frame.pack(side=tk.LEFT, padx=(0, 30), fill=tk.BOTH)  # Increased right padding from 10 to 30
        left_panel_frame.pack_propagate(False)  # Prevent the frame from shrinking
        ...
        left_panel = tk.Frame(left_canvas, bg="#2c3e50", width=330)  # Set width explicitly
        left_canvas.create_window((0, 0), window=left_panel, anchor="nw", tags="left_panel", width=330)
        
        # Formula frame
        formula_frame = tk.LabelFrame(left_panel, text="Physics Formulas", bg="#34495e", fg="#ecf0f1", 
                                    font=("Arial", 12, "bold"), padx=20, pady=10)  # Increased padx from 10 to 20
        formula_frame.pack(padx=10, pady=10, fill=tk.X)
        
        # Add formulas with labels (increased font size from 9 to 12)
        # Fixed formula text with proper formatting and spacing
        formula_text = """
Position:
x(t) = v₀ cos(θ) · t
y(t) = h₀ + v₀ sin(θ) · t - ½gt²
...
t = time
        ...
        formula_label = tk.Label(formula_frame, text=formula_text, justify=tk.LEFT,
                               bg="#34495e", fg="#ecf0f1", font=("Courier", 12))
        ...
        def create_input_row(parent, label_text, variable, unit="", min_val=0, max_val=100, increment=1):
            ...
            scale = tk.Scale(frame, from_=min_val, to=max_val, orient=tk.HORIZONTAL, 
                            ...
                            length=150)  # Set specific length for scale
            scale.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))  # Increased right padding from 5 to 10
            ...
            if unit:
                ...
                unit_label.pack(side=tk.LEFT, padx=(5, 0))  # Increased left padding from 2 to 5
